...
int CSplitterLinkList::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_WndLinkListSplitter.CreateView(0,0,RUNTIME_CLASS(CLinkListDraw), CSize(rect.Width(),rect.Height()/2), pContext);
	m_WndLinkListSplitter.CreateView(1,0,RUNTIME_CLASS(CLinkListCrl), CSize(rect.Width(),rect.Height()/2), pContext);
	return 0;
}
...
void CSplitterLinkList::OnSize(UINT nType, int cx, int cy) 
{
	...
	m_WndLinkListSplitter.SetRowInfo(0, cy-(cy/6), 0);
	m_WndLinkListSplitter.SetRowInfo(1, cy/6, 20);
